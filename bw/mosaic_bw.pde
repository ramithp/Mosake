...
/*
* A sketch to generate brightness scores for images in a folder from 0-255.
* These 256 images are then used to generate a new image that boxes pixels by the pixlFactor
*/
...
PImage[] mosaicTiles;
float[] avgBrightness;
PImage[] imageForBrightness = new PImage[256];
...
void setup(){
  ...
  baseImg = loadImage(mosaicFile);
    ...
    avgBrightness = new float[fileCount];
    ...
    for(int i = 0; i < mosaicTiles.length; i++){
     ...
     for(int j = 0; j < mosaicTiles[i].pixels.length; j= j+1){
      println(getRGBNormalized(mosaicTiles[i].pixels[j]));
       avg += brightness(mosaicTiles[i].pixels[j]);
     }
     avg /= mosaicTiles[i].pixels.length;
     avgBrightness[i] = avg;
  }

  for(int i = 0; i < imageForBrightness.length; i++){
    float record = 256;
    //Iterate through all images to find the one that is closest to
    for (int j = 0; j < avgBrightness.length; j++) {

      float diff = abs(i - avgBrightness[j]);
      if (diff < record) {
        record = diff;
        imageForBrightness[i] = mosaicTiles[j];
      }
    }
  }
}
...
void draw(){
  ...
  for(int x = 0; x < w; x++){
   for(int y = 0; y < h; y++){
     ...
     fill(brightness(c));
     noStroke();
     image(imageForBrightness[imageIndex], x*pixlFactor, y*pixlFactor, pixlFactor, pixlFactor);
